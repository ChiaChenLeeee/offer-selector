...
import { Trophy, ChevronDown, HelpCircle, Star, Calculator, AlertTriangle } from 'lucide-react';
                  ...
                  <tr key={res.offerId} className={`hover:bg-slate-50/30 transition-colors ${isRisk ? 'bg-rose-50/20' : ''}`}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {isRisk && <AlertTriangle className="w-4 h-4 text-rose-500" title="该 Offer 风险评分较高" />}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <td key={dim.id} className="px-6 py-4">
                          <div className="space-y-1">
                            <div className="flex justify-between items-center text-xs">
                              <span className={`font-medium ${score < 0 ? 'text-rose-600' : 'text-slate-600'}`}>{score.toFixed(0)}</span>
                              {bonusObj && (
                            ...
                            <div className="h-1 w-full bg-slate-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full transition-all duration-700 ${dim.isPenalty ? 'bg-rose-500' : score > 80 ? 'bg-emerald-500' : score > 50 ? 'bg-indigo-500' : 'bg-amber-500'}`}
