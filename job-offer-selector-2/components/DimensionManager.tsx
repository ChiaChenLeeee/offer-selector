...
import { 
  ...
  ChevronUp, Sliders, AlertCircle, GripVertical, 
...
const DimensionManager: React.FC<DimensionManagerProps> = ({ 
  ...
  const calculateAutoScores = (options: DimensionOption[], isPenalty: boolean): DimensionOption[] => {
    ...
    return options.map((opt, index) => {
      // Top rank (index 0) is 100/-100, bottom rank (index m-1) is 0
  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200 h-fit">
              ...
              <div className="flex items-center justify-between p-4">
                <div className="flex items-center gap-3">
                  <button
                    ...
                    disabled={dim.id === 'company'} // 仅公司名称不可取消，作为主标识符
      ...
      {showAddForm && (
        <div className="mt-6 p-5 bg-slate-50 rounded-2xl border border-slate-200 animate-in fade-in zoom-in-95">
